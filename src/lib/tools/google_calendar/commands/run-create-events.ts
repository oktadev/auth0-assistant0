...
import { CREATE_EVENT_PROMPT } from '../prompts/index';
...
const runCreateEvent = async (
  ...
  const [eventSummary, eventStartTime, eventEndTime, eventLocation, eventDescription, userTimezone] =
  ...
  return `An error occurred creating the event: ${(event as { error: string }).error}`;
};
