...
export class GmailSendMessage extends GmailBaseTool {
  ...
  private createEmailMessage({ message, to, subject, cc, bcc }: z.infer<typeof this.schema>): string {
    const emailLines: string[] = [];

    // Format the recipient(s)
    const formatEmailList = (emails: string | string[]): string => (Array.isArray(emails) ? emails.join(',') : emails);
  ...
  async _call({ message, to, subject, cc, bcc }: z.output<typeof this.schema>): Promise<string> {
