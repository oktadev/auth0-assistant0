...
export abstract class GmailBaseTool extends StructuredTool {
  ...
  async getGmailClient() {
    ...
    if (credentials?.accessToken) {
      ...
      const accessToken =
        typeof credentials.accessToken === 'function' ? await credentials.accessToken() : credentials.accessToken;
  ...
  parseHeaderAndBody(payload: gmail_v1.Schema$MessagePart | undefined) {
    ...
    if (payload.parts) {
      body = payload.parts
        .map((part) => (part.mimeType === 'text/plain' ? this.decodeBody(part.body?.data ?? '') : ''))
        .join('');
